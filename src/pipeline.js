...
async function run({ testMode = false } = {}) {
  ...
  state = {
    date: dateString,
    rawDataPath: path.join(__dirname, '..', 'raw-data', `${dateString}.json`),
    outputsBase: path.join(
      process.env.OUTPUTS_PATH || path.join(require('os').homedir(), 'Desktop', 'rollin-outputs'),
      dateString
    ),
    scrapedVideos: [],
    scoredVideos: [],
