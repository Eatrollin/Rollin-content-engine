...
// Default: 0 6 1,15 * * = 6:00am on the 1st and 15th of each month, Detroit time
// Examples:
//   0 6 * * *    = 6:00am daily
//   30 5 * * *   = 5:30am daily
//   0 7 * * 1-5  = 7:00am weekdays only
const CRON_SCHEDULE = process.env.CRON_SCHEDULE || '0 6 1,15 * *';

function register() {
  if (!cron.validate(CRON_SCHEDULE)) {
    logger.error(`Invalid CRON_SCHEDULE in .env: "${CRON_SCHEDULE}"`);
    logger.error('Falling back to default: 0 6 1,15 * * (6:00am on the 1st and 15th)');
  }
  ...
  cron.schedule(
    cron.validate(CRON_SCHEDULE) ? CRON_SCHEDULE : '0 6 1,15 * *',
    async () => {
      logger.info('');
      logger.info('══════════════════════════════════════════════════');
